...
class ArnavTocEntry extends kwish_Model {
    ...
    static entriesByPageURI = {};

	constructor(major, id, uniqueId, title, level, uri = undefined) {

		super(major, uniqueId);
        ...
        if (!!uri) {
            ArnavTocEntry.entriesByPageURI[uri] = this;
        }
	}
    ...
    static getTopiIdByPageURI(pageURI) {
        return ArnavTocEntry.entriesByPageURI[pageURI].getId();
    }
}
...
class ArnavDoc extends kwish_Model {
	...
	createEntryOfDto(major, entryDto, level) {
        ...
        const uniqueId = this.assembleTopicUniqueId(id);
	...
	loadFromDto(dto) {
        ...
        this.url =  dto["entry"]["@uri"];

        ArnavTocEntry.entriesByPageURI[this.url] = this;
...
function splitByFirstHash(str) {
    ...
    const normStr = kwish_Utils.dry(str);
...
class SearchEngine extends kwish_Model {
    ...
    calcWordContributionToToken(token, word) {

        if(word.includes(token) && word.length !== 0) {
    ...
    countWordOccurrences(topicId, words) {
        
        const count = words.reduce(
            (sum, word) => {
                const match = this.getSearchIndex().entries[word].find(e => e.i === topicId);
                return sum + (match ? match.m : 0);
            }, 0);
        
        return count;
    }

    matchSearchToken(topicId, token, words) {
       
        const contrib = words.reduce(
            (max, word) => Math.max(
                    max, this.countWordOccurrences(topicId, [word]) > 0 ? this.calcWordContributionToToken(token, word) : 0
                ), 0
            );
    ...
    completeTokens(passedTokens) {

        let stopWords = [];
        stopWords = this.getSearchIndex().stopwords?.split(" ") || [];

        let tokens = null;

        if (this.getApp().hasTaxonomy()) {
            tokens = passedTokens.map(
                t => this.substituteTaxonValue(t.toLowerCase())
            ).filter(t => !stopWords.includes(t));
        } else {
            tokens = passedTokens.map(t => t.toLowerCase()).filter(t => !stopWords.includes(t));
        }

        return tokens;
    }

    detectMatchingWords(tokens) {

        const wordMatchScores = {};
        
        for(const word in this.getSearchIndex().entries) {
            for(const token of tokens) {
                let matchScore = this.calcWordContributionToToken(token, word);
                if(matchScore > 0) {
        ...
        return Object.keys(wordMatchScores);
    }

    detectMatchingTopics(matchingWords) {
        return matchingWords.flatMap(word => this.getSearchIndex().entries[word].map(e => e.i));
    }

    calcRelevantTopicScores(matchingTopics, tokens, matchingWords) {

        const scores = [];

        for (const topicId of matchingTopics) {

            scores[topicId] = {
                "relevance": 0, 
                "rate": this.countWordOccurrences(topicId, matchingWords)
            };
            
            for (const token of tokens) {
                scores[topicId].relevance += this.matchSearchToken(topicId, token, matchingWords);
            }

            scores[topicId].relevance /= tokens.length;
        }
    ...
    compareScores(scoreA, scoreB) {
        return scoreB.relevance - scoreA.relevance || scoreB.rate - scoreA.rate;
    }
    ...
    splitScores(scores) {
        ...
        const sortedKeys = keys.sort((a, b) => this.compareScores(scores[a], scores[b]));
    
        const relevanceDict = {};
        for (const key of sortedKeys) {
            relevanceDict[key] = scores[key].relevance;
        }
    
        return [sortedKeys, relevanceDict];
    }

    assembleTopicUniqueId(topicOriginalId) { 
        return topicOriginalId; 
    }

    searchRelevantTopicIds(passedTokens) {        

        const tokens = this.completeTokens(passedTokens);
        const matchingWords = this.detectMatchingWords(tokens);
        const matchingTopics = this.detectMatchingTopics(matchingWords);
        const scores = this.calcRelevantTopicScores(matchingTopics, tokens, matchingWords);
        
        const scoresWithUniqueIds = {};
        for (const topicOriginalId in scores) {
            scoresWithUniqueIds[this.assembleTopicUniqueId(topicOriginalId)] = scores[topicOriginalId];
        }

        return this.splitScores(scoresWithUniqueIds);
    }

    getTopicInfo(topicId, rating) {
        
        if (!this.entries[topicId]) {
            return {};
        }

        const topicInfo = {
            "id": this.entries[topicId].getId(),
            "title": this.entries[topicId].getTitle(),
            "url": this.entries[topicId].getUri(),
            "taxons": this.getTopicTaxons(topicId),
            "rating": rating[this.entries[topicId].getId()]
        };
    ...
    assembleSearchResults(topicIdsAndRatings) {
        ...
        for(const topicId of topicIds) {
            let topicInfo = this.getTopicInfo(topicId, topicRatings);
            if (!!topicInfo.url) {
...
class ClearButton extends SearchToolButton {
    ...
    handle_userActionRequest(task) {
        ...
        this.proclaimCommand('cancelSearch');
...
class TaxonomyPane extends kwish_DOMControl {
    ...
    makeDOMView() {
        ...
        const div = this.inject(new kwish_BoxShellViewMaker())
...
class Searcher extends kwish_BasicDOMControl {
    ...
    saveSearchQuery(query) {
        this.setCfgParam('search.results.query', query).getCfg().commitUserParams();
        return this;
    }

    loadSearchQuery() {
        return this.getCfgParam('search.results.query');
    }

    restoreSearchState() {
        return this.setSearchQuery(this.loadSearchQuery());
    }
    ...
    getSearchTokens() {
        ...
        const rawSearchTokens = this.splitSearchQuery(query);
    ...
    assembleRelevanceIndicator(relevance) {
        ...
        const stars = [
            this.getRsrcAsset('nullstar'),
            this.getRsrcAsset('halfstar'),
            this.getRsrcAsset('fullstar')
        ];
    ...
    removeMarkup(htmlText) {
        ...
        const matterElementId = this.getCfgParam('matter.framelet.payloadId');
        const usefulText = this.extractUseful(body, matterElementId);

        if (!usefulText) return '';

        const pageHeadingTagName = this.getCfgParam('matter.framelet.pageHeadingTagName');
        const h1 = usefulText.querySelector(pageHeadingTagName);
        if (h1) {
            h1.remove();
        }
    ...
    displayTargetSnippet(tokens, topicId, htmlPageAsText) {
        ...
        for(let i = 0; i < relevantSentences.length; i++) {
            ...
            spanSentence.innerHTML = sentence.trim() + (i < relevantSentences.length - 1 ? " &mdash;&nbsp;" : "");
            for(const token of tokens) {
    ...
    assembleTopicPreview(tokens, topicId) {
        ...
        const entry = this.getGear(topicId);
        
        fetch(entry.uri).then(
            response => {
                if(response.ok) {
                    response.text().then(
    ...
    performSearch() {
        ...
        this.saveSearchQuery(this.getSearchQuery());

    }

    cleanSearch() {
        return this.setSearchQuery('').saveSearchQuery('');
    }
...
class TocShortcut extends kwish_BasicDOMControl {
    ...
    handle_maximize(task) {
        this.show();
    }

    handle_normalize(task) {
        this.hide();
    }

    handle_getMobile(task) {
        this.hide();
    }

    handle_getLargeScreen(task) {

        const state = this.getGear('readerConsole').getEasyFSMState();
    ...
    handle_dom_click(task) {
        this.terminateTask(task).proclaimCommand('normalize');
    }
}
...
class CloseBox extends kwish_BasicDOMControl {
    ...
    handle_dom_click(task) {
        this.terminateTask(task).proclaimCommand('maximize');
    }
}
...
class Breadcrumbs extends kwish_DOMControl {
    ...
    makeDOMView() {
        ...
        for (const entry of treePath) {
            let uri = entry.uri;
            let inner = !!uri ? `<a href="${uri}">${entry.title}</a>` : entry.title;
            crumbsTplItems.push(`<span>${inner}</span>`);
        }
...
class TocTree extends kwish_SelectableTreeControl {
    ...
    handle_userAction(task) {
        ...
        if (!task.initiator.refCode) {
            return this.terminateTask(task);
        }
...
class TocFramelet extends kwish_BasicDOMControl {
    ...
    constructor(major, propsRec, id = undefined) {
        ...
        this.setProps(
            {
                'widthCSSPropName': '--toc_framelet_width',
                'minTocWidth': this.getCfgParam('toc.framelet.minWidth') || 100,
                'restoreIndent': this.getCfgParam('toc.framelet.restoreIndent') || 10
            }
        );

        this.setPrimaryScreenEventTypes(
            ['DOMMouseScroll', 'mousemove', 'mouseup', 'scroll', 'wheel']
        );
    }

    getInitialState() {
        const rawState = this.getCfgParam('toc.framelet.folded');
        return rawState === 'maximized' ? 'folded' : 'normal';
    }
    ...
    isFolded() {
        return this.getEasyFSMState() === 'folded';
    }
    ...
    setWidth(width) {
        kwish_DOMUtils.setCSSProp(this.getWidthCSSPropName(), s(width));
        return this;
    }
    ...
    restoreTocTreeState() {
        ...
        this.tocTree.openRoot().openByTreePaths(tocUnfoldedList); 
        return this;
    }

    highlightCurrentTocEntry() {
        const refCode = this.getApp().getCurrPageFileName();
        this.tocTree.forceHighlight(refCode);
        return this;
    }
    ...
    saveScroll() {
        ...
        this.setCfgParam('toc.framelet.yscroll', scroll).getCfg().commitUserParams();
    ...
    restoreTocPaneState() {
        ...
        this.restoreTocTreeState().highlightCurrentTocEntry();
    ...
    tileNormal() {
        ...
        if (currWidth <= this.getMinTocWidth()) {
            this.setWidth(currWidth + this.getRestoreIndent());
        }

        return this.proclaimCommand('tocFrameletShown');
    }

    handle_prepare(task) {
        this.configureFSMachine().configureMinorControllers();
    }

    handle_restore(task) {

        this.restoreTocPaneState();
    ...
    handle_split(task) {
        const newWidth = task.getPayload();
        this.setWidth(newWidth).proclaimCommand('tocFrameletShown');
    }
    ...
    handle_getLargeScreen(task) {
        this.getEasyFSMState() === 'folded' ? this.hide() : this.show().tileNormal();
    }
...
class PrintButton extends kwish_IconicButton {
    
    constructor(major, props, id = undefined) {
        ...
        super(major, printButtonProps, id);
    }

    performButtonAction(task) {
        this.proclaimCommand('print');
    }
}
...
class MatterFramelet extends kwish_DOMControl {
    ...
    constructor(major, propsRec, id = undefined) {
        ...
        this.setProps(
            {
                'indentCSSPropName': '--major_indent',
                'startCSSPropName': '--matter_framelet_start',
                'topCSSPropName': '--matter_framelet_top',
                'widthCSSPropName': '--matter_framelet_width',
                'heightCSSPropName': '--matter_framelet_height',
            }                               
        );
    }
    ...
    configureFSMachine() {
        this.easyFSMachine(
            `[*] --> ${this.getApp().getInitialState()} 
            
            normal    --> maximized : maximize
            maximized --> normal    : normalize
            
            normal    --> mobileFromNormal    : getMobile
            maximized --> mobileFromMaximized : getMobile
            
            mobileFromNormal    --> normal    : getLargeScreen
            mobileFromMaximized --> maximized : getLargeScreen`
        );
    }

    isNormal() {
        return this.getEasyFSMState() === 'normal';
    }
    ...
    isMobile() {
        const currState = this.getEasyFSMState();
        return currState === 'mobileFromNormal' || currState === 'mobileFromMaximized';
    }

    highlightSubstringInDOM(rootElement, searchTerm) {

        const walker = document.createTreeWalker(
            rootElement, NodeFilter.SHOW_TEXT, null, false
        );
        ...
        nodesToHighlight.forEach(({ node, index, content }) => {
            ...
            bold.className = this.getProp('searchTermCSSClassName');
    ...
    unhighliteSearchTerms() {

        const className = this.getProp('searchTermCSSClassName');
        const body = document.getElementsByTagName('body')[0];
        const highlitedElements = body.querySelectorAll(`.${className}`);

        for(const element of highlitedElements) {
            kwish_DOMUtils.unwrap(element);
        }
    ...
    place(start, top, width, height) {

        kwish_DOMUtils.setCSSProps(
            this.getStartCSSPropName(), s(start),
            this.getTopCSSPropName(), s(top),
            this.getWidthCSSPropName(), s(width),
            this.getHeightCSSPropName(), s(height)
        );
    ...
    markMaximized(mark) {
        this.getDOMView().setAttribute('data-maximized', mark);  
        return this;  
    }

    tileNormal() {
        
        const tocFramelet =  this.getGear('tocFramelet');

        const windowWidth = kwish_DOMUtils.getWindowWidth();

        const indent = kwish_DOMUtils.measureCSSProp(this.getIndentCSSPropName());
        ...
        if (kwish_DOMUtils.isLTR() ) {
            start = tocFramelet.getEnd() + indent;
            width = windowWidth - start - indent; 
        } else {
            start = windowWidth - tocFramelet.getEnd() + indent;
            width = tocFramelet.getEnd() - 2*indent; 
        }

        console.log(`${width} = ${windowWidth} - ${start} - ${indent}`);

        const height = tocFramelet.getHeight();
        
        return this.place(start, top, width, height).markMaximized('no');
    }

    tileMaximized() {    
        return this.place(0, 0, '100vw', '100vh').markMaximized('yes');
    }

    tileMobile() {
        return this.place(0, 0, '100vw', '100vh').markMaximized('no');
    }

    tileResized() {
        
        if (this.isNormal()) {
            return this.tileNormal();
        } 
    ...
    scrollToAnchor(anchor) {
        ...
        const container = this.getDOMView();
    ...
    restoreScroll() {
        ...
        const anchor = ((location.href.match(/[?&]anchor=([^&#]*)/) || [])[1]) || null;
    ...
    print() {
        let hideHTML = document.body.innerHTML;
        const payloadId = this.getCfgParam('matter.framelet.payloadId');
        document.body.innerHTML = document.getElementById(payloadId).innerHTML;
        window.print();
        document.body.innerHTML = hideHTML;
        location.reload();
        return this;
    }
    ...
    handle_restore(task) {
        ...
        this.updateMatter().restoreScroll();
    }
    ...
    handle_tocFrameletShown(task) {
        this.tileNormal();
    }

    handle_stopResize(task) {
        this.tileResized();
    }

    handle_cancelSearch(task) {
        this.unhighliteSearchTerms();
    }
    ...
    handle_dom_click(task) {
        ...
        const target = task.DOMEvent.target;
        ...
        const anchor = a.getAttribute('data-inpage-anchor');
...
class ReaderConsole extends kwish_DOMControl {
    ...
    isMaximized() {
        return this.getConsoleState() === 'maximized';
    }
    ...
    handle_normal_entered(task) {
        ...
        this.commitConsoleState().setCursor('grab'); 
        
        if (this.getApp().isMobileWidth()) {
            this.proclaimCommand('getMobile');
        }
    }

    handle_maximized_entered(task) {
        this.commitConsoleState();
    }

    handle_startingSplit_entered(task) {
        this.initTocWidth = this.getTocFramelet().getWidth();
    ...
    handle_splitting_entered(task) { 
        ...
        this.tocFramelet.do('split', newTocWidth);
        
        this.commitConsoleState();

        if (this.getTocFramelet().getWidth() < this.getMinTocWidth()) {
            this.proclaimCommand('maximize');
        }
    }

    handle_normalResized_entered(task) {
        this.proclaimCommand(
            this.getApp().isMobileWidth() ? 'getMobile' : 'stopResize'
        );
    }
    ...
    handle_mobileFromNorResized_entered(task) {
        this.proclaimCommand(
            this.getApp().isMobileWidth() ? 'stopResize' : 'getLargeScreen'
        );
    }
...
class Page extends kwish_Page {
    ...
    constructor(major, props, id = undefined) {
        ...
        this.setProps(
            {
                'searchTermCSSClassName': 'search_term'
            }
        );
    }
...
class App extends kwish_App {
    ...
    constructor(major, props, id) {
        ...
        this.setProps( 
            {
                'maxMobileWidth': this.getCfgParam('responsive.maxMobileWidth') || 480
            }
        )
    }

    isMobileWidth() {
        return kwish_DOMUtils.getWindowWidth() <= this.getMaxMobileWidth();
    }

    getInitialState() {

        const savedState = this.getCfgParam('toc.framelet.folded') || 'normal';

        if (this.isMobileWidth()) {
            switch (savedState) {
                case 'normal': return 'mobileFromNormal';
                case 'maximized': return 'mobileFromMaximized';
            }
        } else {
            switch (savedState) {
                case 'mobileFromNormal': return 'normal';
                case 'mobileFromMaximized': return 'maximized';
            }
        }
    ...
    mountPage() {
        this.page = new Page(this).usePresetControls();
        return this;
    }
    ...
    hasTaxonomy() {
        return !!GLOBAL_DITATOO_SEARCH_INDEX_DTO.taxonomy && this.getCfgParam('search.taxonomy.mode') === 'on';
    }
    ...
    getCurrPageFileName() {
        ...
        let stage0 = location.href;

        let stage1 = stage0.includes('#') ? kwish_Utils.substringBefore(stage0, '#') : stage0;
        let stage2 = stage1.includes('?') ? kwish_Utils.substringBefore(stage1, '?') : stage1;

        return kwish_Utils.substringAfterLast(stage2, '/');
    }

    getTopicIdByPageFileName(pageURL) {
        const doc = this.getModel();
        //console.log('::: ', doc);
        return ArnavTocEntry.getTopicIdByPageFileName(pageURL);
    }

    getCurrPageId() {

        const currPageId = ArnavTocEntry.getTopiIdByPageURI(this.getCurrPageFileName());
    ...
    goOn() {
        this.mountDoc().mountPage();
        this.tossDownCommand('prepare');
        this.page.appear();
        this.tossDownCommand('restore');
    }
...
function ensureAnchor() {
    ...
    if (!hash || url.searchParams.has('anchor')) return;
