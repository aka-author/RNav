...
class TocShortcut extends kwish_BasicDOMControl {
    ...
    handle_dom_click(task) {
        this.proclaimCommand('normalize');
        /*
...
class TocFramelet extends kwish_BasicDOMControl {
    ...
    tileNormal() {
        ...
        if (currWidth <= this.getMinTocWidth()) {
            const indent = this.getRestoreIndent();
            const correctedWidth = p(currWidth + indent);
            this.setWidth(correctedWidth);
        }
...
class MatterFramelet extends kwish_DOMControl {
    ...
    tileNormal() {
        ...
        return this.place(start, top, width, height);
    }

    tileMaximized() {    
        return this.place(0, 0, '100vw', '100vh');
    }
    ...
    tileResized() {
        switch (this.getEasyFSMState()) {
            case 'normal': 
                this.tileNormal();
                break;
            case 'mobileFromNormal':
            case 'mobileFromMaximized':
                this.tileMobile();
        }
    }
    ...
    maximize() {
        this.tileMaximized().markMaximized('yes');
    }

    normalize() {
        this.markMaximized('no');
    }
...
class ReaderConsole extends kwish_DOMControl {
    ...
    handle_splitting_entered(task) { 
        ...
        this.tocFramelet.do('split', newNavWidth);
    ...
    handle_normalResized_entered(task) {
        this.proclaimCommand(this.getApp().isMobileWidth() ? 'getMobile' : 'stopResize');
    }
    ...
    handle_mobileFromNorResized_entered(task) {
        this.proclaimCommand(this.getApp().isMobileWidth() ? 'stopResize' : 'getLargeScreen');
    }
