...
class TocFramelet extends kwish_BasicDOMControl {
    ...
    setWidth(width) {
        kwish_DOMUtils.setCSSProp(this.getWidthCSSPropName(), width);
    }
    ...
    tileNormal() {
        ...
        if (this.getWidth() <= this.getMinTocWidth()) {
            this.setWidth(`${this.getWidth() + 10}px`)
        }
    ...
    handle_getLargeScreen(task) {
        if (this.getEasyFSMState() === 'normal') {
            this.show().tileNormal();
        }
    }
...
class MatterFramelet extends kwish_DOMControl {
    ...
    getInitialState() {
        return this.getCfgParam('toc.framelet.folded') || 'normal';
    }
    ...
    place(start, top, width, height) {
        kwish_DOMUtils.setCSSProps(
            this.getStartCSSPropName(), start,
            this.getTopCSSPropName(), top,
            this.getWidthCSSPropName(), width,
            this.getHeightCSSPropName(), height
        );
        return this;
    }
    ...
    tileNormal() {
        ...
        return this.place(p(start), p(top), p(width), p(height));
    }
    ...
    handle_stopResize(task) {
        this.getEasyFSMState() === 'normal' ? this.tileNormal() : this.tileMaximized();
    }
}
...
class ReaderConsole extends kwish_DOMControl {
    ...
    tileTwoFramelets() {
        ...
        let navWidth = navf.getWidth();
        if (navWidth <= this.getMinTocWidth()) {
            navWidth += 10;
        }
        ...
        /*kwish_DOMUtils.setCSSProps(
            this.getNavigationWidthCSSPropName(), p(navWidth),
            this.getContentStartCSSPropName(), p(cntStart),
            this.getContentTopCSSPropName(), p(cntTop),
            this.getContentWidthCSSPropName(), p(cntWidth), 
            this.getContentHeightCSSPropName(), p(cntHeight)
        );*/
    ...
    handle_normalResized_entered(task) {
        this.proclaimCommand(this.isMobileSize() ? 'getMobile' : 'stopResize');
    }
    ...
    handle_mobileFromNorResized_entered(task) {
        this.proclaimCommand(this.isMobileSize() ? 'stopResize' : 'getLargeScreen');
    }
