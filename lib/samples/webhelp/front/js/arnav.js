...
class TwoFrameletConsole extends kwish_DOMControl {
    ...
    commitConsoleState() {
        ...
        const tocFremeletSize = `${tocFrameletWidth}/${windowWidth}`;
        const folded = this.getEasyFSMState();

        this.setCfgParam('toc.framelet.size', tocFremeletSize)
            .setCfgParam('toc.framelet.folded', folded)
            .getCfg().commitUserParams();

        /*
        localStorage.setItem(`${keyBase}_tfw`, this.getTocFramelet().getWidth());
        localStorage.setItem(`${keyBase}_mfs`, this.getMatterFramelet().getWidth());
...
class ReaderConsole extends TwoFrameletConsole {
    ...
    handle_maximized_entered(task) {
        ...
        this.terminateTask(task);
        this.setCfgParam('toc.framelet.folded', this.getEasyFSMState());
    }
...
(function () {
    ...
    const tocFrameletFolded = kwish_Cfg.getFromLocalStorage(keyBase, 'toc.framelet.folded');

    console.log(tocFrameletFolded);
