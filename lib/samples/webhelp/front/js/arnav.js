...
class TaxonomyPane extends kwish_DOMControl {
    ...
    makeDOMView() {
        ...
        divPane.setAttribute('id', 'taxonomyPane'); 
        divPane.setAttribute('style', 'display: none');
...
class MatterFramelet extends kwish_DOMControl {
    ...
    updateMatter() {
        ...
        for(const token of tokens) {
            this.highlightSubstringInDOM(document.getElementById('topicBody'), token);
        }
    }
}
...
class TwoFrameletConsole extends kwish_DOMControl {
    ...
    commitConsoleState() {
        const keyBase = this.getApp().getId();
        localStorage.setItem(`${keyBase}_tfw`, this.getTocFramelet().getWidth());
        localStorage.setItem(`${keyBase}_sww`, window.innerWidth);
    }
...
class App extends kwish_App {
    ...
    restoreSearcherState() {
        ...
        //this.searcher.searchInput.getDomObject().value = query;
        return this;
    }
...
function importTree(toc, strings) {
    ...
    function buildNode(entry, prefix = '') {
        ...
        const node = {
            'title': title,
            'number': index,  // Separate number property
            'state': 'hidden'
        };
...
function importTaxonomy(taxonomy) {
    ...
    return {
        title: taxonomy.name || "",
        nodes: taxonomy.taxa.map(convertNode)
    };
}

function assembleAppKey() {
    return `help_${kwish_Utils.hashObject(GLOBAL_DOC_DTO)}`;
}

(function correctCSS() {

    const keyBase = assembleAppKey();
    const tocFramelerWidthKey = `${keyBase}_tfw`;
    const tocWindowWidthKey = `${keyBase}_sww`;
    const storedTocFrameletWidth = localStorage.getItem(tocFramelerWidthKey);
    const storedWindowWidth = localStorage.getItem(tocWindowWidthKey);
    const actualWindowWidth = window.innerWidth;
    
    if(storedWindowWidth == actualWindowWidth && !!storedTocFrameletWidth) {
        document.documentElement.style.setProperty(
            '--toc_framelet_default_width', `${storedTocFrameletWidth}px`
        );
    }
})();
...
function main() {
    ...
    const taxonomyRec = importTaxonomy(GLOBAL_SEARCH_INDEX.taxonomy);
    ...
    const appId = `help_${kwish_Utils.hashObject(GLOBAL_DOC_DTO)}`;
    var WH_GLOBAL_APP = new App(propsRec, appId).start(staticCfgRec, staticRsrcRec);
}
