...
class SearchEngine extends kwish_Model {
    ...
    searchRelevantTopicIds(searchTokens) {
        ...
        if (this.getApp().hasTaxonomy()) {
            rawTokens = searchTokens.map(
                t => this.substituteTaxonValue(t.toLowerCase())
            ).filter(t => !stopWords.includes(t));
        } else {
            rawTokens = searchTokens.map(t => t.toLowerCase()).filter(t => !stopWords.includes(t));
        }
        ...
        for(const word of matchingWords) 
            for(const topicEntry of searchIndex.entries[word]) {
                ...
                relevanceScores[topicId] = kwish_Utils.safeInc(relevanceScores[topicId], wordMatchScores[word]);
            }
