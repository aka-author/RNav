...
class kwish_Bureaucrat extends kwish_Vertex {
    ...
    do(taskTypeName, payload = null) {
        return this.doCommand(taskTypeName, kwish_TASK_PRIVAT, payload);
    }

    tossUpCommand(taskTypeName, payload = null) {
        return this.escalateCommand(taskTypeName, kwish_TASK_BUBBLE, payload);
    }

    tossDownCommand(taskTypeName, payload = null) {

        const task = this.createTask(taskTypeName);

        task.setPropagationStrategyCode(kwish_TASK_SINKER)
            .setPayload(payload);

        return this.delegateTask(task);
    }

    proclaimCommand(taskTypeName, payload = null) {
        return this.doCommand(taskTypeName, kwish_TASK_PUBLIC, payload);
    }
