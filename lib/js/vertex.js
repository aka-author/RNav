...
class RNav_Vertex extends RNav_Generic {
    ...
    getProp(propName) {
        ...
        if(!this.hasUsefulProp(propName) && !this.hasUpper())
            return this.getUpper().getProp(propName);
        else
    ...
    getByIndex(idx) {
        return this.getOne(Object.keys(this.underVertices)[idx]);
    }
