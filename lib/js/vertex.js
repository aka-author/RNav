...
class kwish_Vertex extends kwish_Gear {
    ...
    addMinors(...verteces) {
        verteces.map(v => this.addMinor(v));
        return this;
    }
    ...
    getVertexPath() {
        const majorVertexPath = this.hasMajor() ? `${this.getMajor().getVertexPath()}` : '';
        const localVertexPath = `${kwish_Utils.dernav(this.getClassName())}#${this.getId()}`;
        return `${majorVertexPath}/${localVertexPath}`;
    }

    // Private interface

    getResrc(rsrcCode) {
        return kwish_Rsrc.getRsrc().getAsset(rsrcCode, this.getVertexPath());
    }
