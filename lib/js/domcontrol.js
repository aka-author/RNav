...
class kwish_DOMControl extends kwish_Controller {
    ...
    constructor(major = null, propsRec = {}, id = undefined) {
        ...
        const defaultPropsRec = {
            ...
            'primaryViewMaker': null,
            'defaultSpotName': undefined,
            'defaultSpot': null,
            'DOMView': null,
            'primaryScreenEventTypes': []
        }
    ...
    show() {
        kwish_DOMUtils.show(this.getDOMView());
        return this;
    }

    hide() {
        kwish_DOMUtils.hide(this.getDOMView());
        return this;
    }

    bringToFront() {
        kwish_DOMUtils.bringToFront(this.getDOMView());
        return this;
    }

    getStart() {
        return kwish_DOMUtils.getStart(this.getDOMView());
    }

    getWidth() {
        return kwish_DOMUtils.getWidth(this.getDOMView());
    }

    getEnd() {
        return kwish_DOMUtils.getEnd(this.getDOMView());
    }

    getTop() {
        return kwish_DOMUtils.getTop(this.getDOMView());
    }

    getHeight() {
        return kwish_DOMUtils.getHeight(this.getDOMView());
    }

    getBottom() {
        return kwish_DOMUtils.getBottom(this.getDOMView());
    }

    setCursor(cursorCode) {
        kwish_DOMUtils.setCursor(this.getDOMView(), cursorCode);
        return this;
    }

    getDefaultSpotName() {
        return this.defaultSpotName || this.primaryViewMaker?.getDefaultSpotName();
    }
    ...
    isInRespArea(event) {
        const respArea = kwish_DOMUtils.getGlobalRect(this.getDOMView());
        const eventVect = kwish_DOMUtils.getEventGlobalVect(event);
        return respArea.belongs(eventVect);
    }

    isDirectScreenEventAccepter(event) {
            
        for (const minor of this) {
            if (minor.isInRespArea(event)) {
                return false;
            }
        }
    ...
    isPrimaryScreenAcceptor(event) {

        if (!event) return false;
        
        return this.getPrimaryScreenEventTypes().includes(event.type);
    }

    verifyDOMEvent(event) {

        if (kwish_DOMUtils.isScreenEvent(event)) {
            return this.isDirectScreenEventAccepter(event) || 
                   this.isPrimaryScreenAcceptor(event);
        }
    ...
    installEventListners(domView) {
        ...
        for (const domEventName of domEventNames) {
            if (this.hasTaskHandler(this.assembleDOMTaskTypeName(domEventName))) {
                domView.addEventListener(domEventName, e => this.onDOMEvent(e));
            }
        }
    ...
    mountDOMView() {
        ...
        const myDOMView = this.provideDOMView()
        if (kwish_DOMUtils.isDOMElement(myDOMView)) { 
