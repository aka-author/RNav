...
class kwish_Utils {
  ...
  // Struggling with nulls and undefineds

  /**
   * Safely increments a number by a given value.
   * If the input number is undefined, the function returns the increment itself.
   *
   * @param {number|undefined} n - The number to increment. Can be undefined.
   * @param {number} [increment=1] - The value to increment by. Defaults to 1.
   * @returns {number} The incremented value, or the increment itself if the input was undefined.
   */
  static safeInc(n, increment = 1) {
  ...
  static hasHashtag(_str, hashtag) {
    const str = _str.trim();
  ...
  static safeHashtag(_str, hashtag) {

    const tag = `#${hashtag}`.trim();
    const str = _str.trim();
    const hashtagPattern = /(#\S+(\s#\S+)*)$/;
    const match = str.match(hashtagPattern);
    const existingTags = match ? match[0].split(' ') : [];
...
static unhashtag(_str, hashtag) {
    const tag = `#${hashtag}`.trim();
    const str = _str.trim();
    const tagPattern = new RegExp(`(\\s*${tag}\\b)`, 'g');
    const result = str.replace(tagPattern, '').trim();
    return result;
}
