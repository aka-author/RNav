...
class RNav_Utils {
   ...
   */
  static getShortId() {
    return `i${RNav_Utils.compressUUID(RNav_Utils.getUUID4())}`;
  }
  ...
  /**
   * Removes the "RNav_" prefix from a string if present.
   * 
   * @param str The string to process
   * @return The string with the "RNav_" prefix removed if it was present, otherwise the original string
   */
  static dernav(str) {
    const prefix = 'RNav_';
  ...
  static deprefix(str, prefix = undefined) {
    ...
    if (!!prefix) {
      return str.startsWith(prefix) ? RNav_Utils.substringAfter(str, prefix) : str;
    }

    return str.includes('_') ? RNav_Utils.substringAfter(str, '_') : str;
  }
  ...
  */
  ...
  static isUsefulObj(obj) {
    if (!obj) return false;
    return typeof obj === 'object' && RNav_Utils.hasProps(obj);
  }
  ...
  static allClassMethodNames(classObject) {
    const propNames = RNav_Utils.allClassPropertyNames(classObject);
  ...
  static parseObject(inObj, parseFunc, path = '') {
    ...
    for (const [key, value] of Object.entries(inObj)) {
        ...
        if (processedValue === Symbol.for('parse')) {
            outObj[key] = RNav_Utils.parseObject(value, parseFunc, newPath);
        } else {
