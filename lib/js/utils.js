...
class RNav_Utils {
  ...
  static camelToSnake(str) {
    return RNav_Utils.decapitalizeFirst(str)
        .replace(/([A-Z])/g, '_$1')
        .toLowerCase();
  }

  static dernav(str) {
    return str.startsWith('RNav_') ? str.slice(4) : str;
  }
  ...
  static parseObject(inObj, parseFunc, path = '') {
    ...
    if (Array.isArray(inObj)) {
        let count = 0;
        for(let propValue of inObj) {
            const newPath = `${path}[${count}]`;
            count++;
            let outValue = parseFunc(propValue, newPath);
            if (typeof outValue === 'symbol') {
                if (outValue === Symbol.for('parse')) {
                    outObj.push(
                      RNav_Utils.parseObject(propValue, parseFunc, newPath)
                    );
                }
            } else 
                outObj.push(outValue);
        }
    } else {
        for(let propName of Object.keys(inObj)) {
            const newPath = `${path}/${propName}`;
            let outValue = parseFunc(inObj[propName], newPath);
            if(typeof outValue === 'symbol') {
                if(outValue === Symbol.for('parse')) {
                    outObj[propName] = RNav_Utils.parseObject(
                      inObj[propName], parseFunc, newPath
                    );
                }
            } else 
                outObj[propName] = outValue;
        }
    }
