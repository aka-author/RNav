...
class kwish_Cfg {
    ...
    hasRemoteParams() {
        return kwish_Utils.isNotEmpty(this.remoteParamGroups);
    }
    ...
    extractGroupName(paramFullName, paramName) {
        return kwish_Utils.substringBefore(paramFullName, this.separName(paramName));
    }
    ...
    static getFromLocalStorage(appKey, paramFullName) {

        let objCfg =  null;

        const jsonCfg = localStorage.getItem(`${appKey}_Cfg`);

        try {
            objCfg = JSON.parse(jsonCfg);
        } catch {
            objCfg = {}
        } 

        return objCfg[paramFullName];
    }
    ...
    isMobile() {
      ...
      static setCustomCreateCfgFunc(cccf) {
        kwish_Cfg.customCreateCfgFunc = cccf;
      }

      static getCfg() {

        if(!kwish_Cfg.cfg)
            kwish_Cfg.cfg = kwish_Cfg.customCreateCfgFunc();
        
        return kwish_Cfg.cfg;
      }
}