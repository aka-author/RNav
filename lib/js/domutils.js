...
class kwish_DOMUtils {
    ...
    /**
     * Converts a numeric value to a pixel string or returns a string with existing units as-is.
     *
     * If the input is a number, it appends "px" to create a pixel value string (e.g., 100 → "100px").
     * If the input is already a string (e.g., "100vh", "10em"), it returns the string unchanged.
     *
     * @param {number|string} val - The size value, either as a number (pixels) or a string with units.
    ...
    static setCSSProps(...pairs) {
        ...
        for (let i = 0; i < pairs.length; i += 2) {
            kwish_DOMUtils.setCSSProp(pairs[i], pairs[i + 1]);
        }
    }
    ...
    static show(domElement) {
        ...
        if (!domElement.hasAttribute('style')) {
            domElement.setAttribute('style', 'display: unset');
        }
    ...
    /**
     * Replaces the specified element with its child nodes, preserving their order and structure.
     * This effectively removes the element from the DOM while keeping its content in place.
     *
     * @param {Element} element - The DOM element to unwrap.
     */
    ...
    /**
     * Returns the horizontal start position of a DOM element based on its text direction.
     * If the element's direction is left-to-right (LTR), it returns the left edge.
     * If the direction is right-to-left (RTL), it returns the right edge.
     *
     * @param {HTMLElement} domElement - The DOM element whose start position is to be calculated.
     * @returns {number} The pixel coordinate of the start edge (left for LTR, right for RTL) relative to the viewport.
     */
    ...
    /**
     * Returns the horizontal end position of a DOM element based on its layout direction.
     * The end position is calculated as the start position plus the element's width.
     * This function assumes the start position is determined by layout direction (LTR or RTL).
     *
     * @param {HTMLElement} element - The DOM element whose end position is to be calculated.
     * @returns {number} The pixel coordinate of the end edge of the element relative to the viewport.
     */
    static getEnd(element) {
        /*
        return kwish_DOMUtils.getStart(element) + kwish_DOMUtils.getWidth(element);
        */

        const rect = element.getBoundingClientRect();
        const dir = getComputedStyle(element).direction;
        
        return dir === "rtl" ? rect.left : rect.right;
    }
    ...
    getRect(element) {
        return new kwish_Rect()
            .setLeft(kwish_DOMUtils.getLeft(element))
            .setTop(kwish_DOMUtils.getTop(element))
            .setWidth(kwish_DOMUtils.getWidth(element))
            .setHeight(kwish_DOMUtils.getHeight(element));
    }
    ...
    /**
     * Brings the given DOM element forward by assigning it the highest z-index among its siblings.
     * If the element already has the maximum z-index, no changes are made.
     *
     * @param {HTMLElement} domElement - The DOM element to bring forward.
     */
    static bringToFront(domElement) {

        if (!domElement || !domElement.parentElement) return;
    
        const siblings = Array.from(domElement.parentElement.children);
        let maxZIndex = 0;
        ...
        const currentZIndex = parseInt(window.getComputedStyle(domElement).zIndex, 10);
        if (isNaN(currentZIndex) || currentZIndex < maxZIndex) {
            domElement.style.zIndex = maxZIndex + 1;
        }
    }    
    ...
    static getGlobalRect(domElement) {
        ...
        return new kwish_Rect(
            new kwish_Vect(left, top), 
            new kwish_Vect(left + width, top + height)
        );
    }
    ...
    static dom(srcHTML) {
        return kwish_DOMUtils.parseSnippet(srcHTML)[0];
    }
    ...
    static getEventGlobalVect(event) {
       
        const x = event.clientX + window.scrollX;
        const y = event.clientY + window.scrollY;
    
        return new kwish_Vect(x, y);
    }
