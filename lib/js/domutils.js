...
class RNav_DOMUtils {
    ...
    static parseDocument(htmlDocument, spotAttrName = 'data-rnav-spot') {
        ...
        const spots = domDoc.querySelectorAll[`[${spotAttrName}]`];

        spots.forEach(spot => spots[spot.getAttribute(spotAttrName)] = spot);

        return [domDoc, spots];
    }

    static parseSnippet(htmlSnipet) {
        const domNodes = [];
        const [tmpDoc, spots] = RNav_DOMUtils.parseDocument(htmlSnipet);
