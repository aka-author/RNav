...
class RNav_DOMUtils {
     ...
     * - `RNav_DOMUtils.recalcWidth` – An object storing width conversion factors.
     * - `RNav_DOMUtils.recalcHeight` – An object storing height conversion factors.
     */
    static mountRecalculationTable() {
        ...
        RNav_DOMUtils.recalcWidth = {};
        RNav_DOMUtils.recalcHeight = {};

        const units = ["cm", "em", "in", "mm", "pt", "px", "vh", "vw"];

        for (const unit of units) {
            dummyElement.style.width = `1${unit}`;
            RNav_DOMUtils.recalcWidth[unit] = dummyElement.offsetWidth;

            dummyElement.style.height = `1${unit}`;
            RNav_DOMUtils.recalcHeight[unit] = dummyElement.offsetHeight;
        }
    ...
    static updateScreenInfo() {
        RNav_DOMUtils.mountRecalculationTable();
    }
    ...
    static applyCSSHighlight(domElement) {
        RNav_DOMUtils.toggleCSSClass(domElement, 'highlighted')
        return domElement;
    }
    ...
    static mergeOrAppend(dstDOMElm, srcDOMElm) {
        ...
        if (dstDOMElm.tagName === srcDOMElm.tagName) {
            ...
            RNav_DOMUtils.moveAllChildNodes(dstDOMElm, srcDOMElm);
        } else {
     ...
     * @param {string} [spotAttrName='data-rnav-spot'] - The attribute name used to identify elements of interest.
     ...
     * @returns {RNav_Vect} - The local vector representing the event position relative to the element.
     */
     ...
     * @returns {RNav_Vect} - The vector representing the difference in position.
     */
