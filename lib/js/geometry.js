...
class RNav_Shape extends RNav_Generic {
    ...
    debugDisplay(containerElement, zIndex=1) {
        ...
        RNav_DOMUtils.makeAbsolute(debugDomObj);
        RNav_DOMUtils.setBackgroundColor(debugDomObj, RNav_DOMUtils.randomColor());
        RNav_DOMUtils.setZIndex(debugDomObj, zIndex);

        containerElement.appendChild(debugDomObj);

        const left = RNav_DOMUtils.getLeft(debugDomObj);
        const top = RNav_DOMUtils.getTop(debugDomObj);

        RNav_DOMUtils.setTitle(debugDomObj, `(${left}, ${top})`);
...
class RNav_Vect extends RNav_Shape {
    ...
    add(vect) {
        return new RNav_Vect(this.x + vect.x, this.y + vect.y);
    }
    ...
    debugAssemble() {
        ...
        RNav_DOMUtils.setRect(circle, this.x - 2, this.y - 2, 5, 5);
...
class RNav_Region extends RNav_Shape {
    ...
    getBoundingRect() {
        return new RNav_Rect(new RNav_Vect(0, 0), new RNav_Vect(0, 0));
    }
...
class RNav_Rect extends RNav_Region {
    ...
    debugAssemble() {
        ...
        RNav_DOMUtils.setRect(rect, this.getLeft(), this.getTop(), this.getWidth(), this.getHeight());
        return rect;
    }
}


class RNav_Circle extends RNav_Region {
    ...
    getBoundingRect() {
        const nail1 = new RNav_Vect(this.center.x - this.r, this.center.y - this.r);
        const nail2 = new RNav_Vect(this.center.x + this.r, this.center.y + this.r);
        return new RNav_Rect(nail1, nail2);
    }
    ...
    debugAssemble() {
        ...
        RNav_DOMUtils.setRect(circle, br.getLeft(), br.getTop(), br.getWidth(), br.getHeight());
        RNav_DOMUtils.setBorderRadius(circle, this.r);
...
class RNav_Periphery extends RNav_Region {
...
class RNav_FramePeriphery extends RNav_Periphery {
    ...
    encloseInnerRect(innerRect, peripheryWidth) {
        ...
        const innerLeftTop = new RNav_Vect(innerRect.getLeft(), innerRect.getTop());
        const innerRightBottom = new RNav_Vect(innerRect.getRight(), innerRect.getBottom()); 
        this.inner = new RNav_Rect(innerLeftTop, innerRightBottom);
        ...
        const outerLeftTop = new RNav_Vect(outerLeft, outerTop);
        const outerRightBottom = new RNav_Vect(outerRight, outerBottom);
        this.outer = new RNav_Rect(outerLeftTop, outerRightBottom);
...
class RNav_WasherPeriphery extends RNav_Periphery {
    ...
    encloseInnerRect(innerRect, diagRatio) {
        ...
        this.outer = new RNav_Circle(this.inner.getCenterVect(), r);
...
class RNav_PlotPlan extends RNav_Root {
    ...
    addZone(shape, ...tags) {
        ...
        const zone = {"shape": shape, "tags": new RNav_Tags(...tags)};
    ...
    addHeaderZone(height) {
        this.addZone(
            new RNav_Rect(
                new RNav_Vect(0, 0), 
                new RNav_Vect(this.getWidth(), height)), "header");
    ...
    addHedgeZone(cornerLength, borderWidth, clientRect) {
        ...
        const top = new RNav_Rect(new RNav_Vect(ltx+c, lty-b), new RNav_Vect(rtx-c, rty+0));
        const bottom = new RNav_Rect(new RNav_Vect(lbx+c, lby), new RNav_Vect(rbx-c, rby+b));
        const left = new RNav_Rect(new RNav_Vect(ltx-b, lty+c), new RNav_Vect(ltx+0, lby-c));
        const right = new RNav_Rect(new RNav_Vect(rtx, rty+c), new RNav_Vect(rbx+b, rby-c));
        ...
        if(c > 0) {
            ...
            const left_top_horz = new RNav_Rect(new RNav_Vect(ltx-b, lty-b), new RNav_Vect(ltx+c, lty));
            const left_top_vert = new RNav_Rect(new RNav_Vect(ltx-b, lty-b), new RNav_Vect(ltx, lty+c));
            const right_top_horz = new RNav_Rect(new RNav_Vect(rtx+b, rty-b), new RNav_Vect(ltx+c, rty));
            const right_top_vert = new RNav_Rect(new RNav_Vect(rtx+b, rty-b), new RNav_Vect(rtx, rty+c));
            const left_bottom_horz = new RNav_Rect(new RNav_Vect(lbx-b, lby+b), new RNav_Vect(lbx+c, lby));
            const left_bottom_vert = new RNav_Rect(new RNav_Vect(ltx-b, lby+b), new RNav_Vect(ltx, rty-c));
            const right_bottom_horz = new RNav_Rect(new RNav_Vect(rtx+b, rby+b), new RNav_Vect(rtx-c, rby));
            const right_bottom_vert = new RNav_Rect(new RNav_Vect(rtx+b, rby+b), new RNav_Vect(rtx, rby-c));
    ...
    addFramePeripheryZone(hedgeWidth, peripheryWidth, clientRect) {
        ...
        const innerRect = new RNav_Rect(new RNav_Vect(innerLeft, innerTop), new RNav_Vect(innerRight, innerBottom));

        const framePeriphery = new RNav_FramePeriphery(innerRect, peripheryWidth);
