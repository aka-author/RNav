...
class PrintButton extends RNav_IconicButton {
    ...
    constructor(major, id) {
        ...
        this.easyFSMachine(
            `[*] --> enabled
            disabled --> enabled : enablePrint
            enabled --> disabled : disablePrint`
        );
    }

    handle_enabled_entered(task) {
        this.showIcon('enabled');
        this.terminateTask(task);
    }
                ...
                'icons': [
                    {
                        'spot': 'enabled', 
                        'alt': 'Print (enabled)',
                        'file': 'print-enabled.svg'
                    },
                    {
                        'spot': 'disabled', 
                        'alt': 'Print (disable)',
                        'file': 'print-disabled.svg'
                    }
                ]
            };

        new PrintButton(this, 'printButton')
            .setIconsPath('img')
            .setIconDescs(iconDescs)
            .setVisibleIconName('enabled');
